...
let find dir =
  let open P in
  printf "\ntmp dir contents:\n"
  >> chdir dir (run "find" [] |- run "sort" [] |- iter_lines (printf "- %s\n"))
  >> printf "\n"
;;
...
let rename_current_directory =
  ...
  with_temp_dir ~prefix:"shexp_process_test" ~suffix:tmpdir_suffix (fun tmpdir ->
    chdir
      tmpdir
      (mkdir "blah"
...
let rm_rf =
  let open P in
  with_temp_dir ~prefix:"shexp_process_test" ~suffix:tmpdir_suffix (fun tmpdir ->
    chdir tmpdir (mkdir "test" >> rm_rf "test" >> find tmpdir))
;;
