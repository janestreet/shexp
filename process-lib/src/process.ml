...
module Context = struct
  ...
  let use_env t =
    with_lock t.mutex ~f:(fun () ->
      match t.state with
      | Disposed -> failwith "Shexp_process: trying to use a disposed context"
      | Active env ->
        Env.add_cwd_ref env;
        env)
  ;;
...
module Temp0 = struct
  ...
  let with_open_temp ~prefix ~suffix ~f =
    ...
    protectx
      ~finally
      ~f
      (create ~temp_dir:(Filename.get_temp_dir_name ()) ~prefix ~suffix ~mk:(fun fn ->
         Unix.openfile fn [ O_WRONLY; O_CREAT; O_EXCL ] 0o600))
  ;;
end
...
let deref_cwd env ~can_deref_cwd = if can_deref_cwd then Env.deref_cwd env else ()
...
let rec exec : type a. Env.t -> can_deref_cwd:bool -> a t -> a =
  ...
  | Bind (t, f) ->
    ...
    then (
      ...
      | exception exn ->
        Env.deref_cwd env;
        reraise exn
      | t -> exec env t ~can_deref_cwd:true)
    else (
  ...
  | Fold { fold; f; init; _ } ->
    ...
    then (
      match
        fold env ~init ~f:(fun acc elt -> exec env (f acc elt) ~can_deref_cwd:false)
      with
      | res ->
        Env.deref_cwd env;
        res
      | exception exn ->
        Env.deref_cwd env;
        reraise exn)
    else fold env ~init ~f:(fun acc elt -> exec env (f acc elt) ~can_deref_cwd:false)
  | Fork (a, b) ->
    if might_block a && might_block b
    then (
      ...
      a_res, b_res)
    else if can_deref_cwd
    then (
      match exec env a ~can_deref_cwd:false with
      | exception exn ->
        Env.deref_cwd env;
        reraise exn
      | a_res -> a_res, exec env b ~can_deref_cwd:true)
    else (
...
module With_debug (D : Debugger) = struct
  let chdir_prim = Prim.make "chdir" [ A sexp_of_string ] Env (fun _ _ -> assert false)
  ...
  module D = struct
    ...
    let need_checkpoint t =
      if t.checkpoint = Create_if_needed
      then (
        D.enter_sub t.dbg;
    ...
    let enter_new_thread_with_capture env dbg ~parent_capture ~f =
      ...
      in
      if not (replace_stdin || replace_stdout || replace_stderr)
      then f env { dbg; capture = None; checkpoint = Not_needed }
      else
        Temp0.with_open_temp
      ...
      protectx
        ...
        ~f:(fun (dbg_a, dbg_b) ->
          ...
          | None ->
            f
              env
              { t with dbg = dbg_a; checkpoint = Not_needed }
              env
              { t with dbg = dbg_b; checkpoint = Not_needed }
          | Some _ ->
            enter_new_thread_with_capture
              env
              dbg_b
    ...
    let toplevel env dbg ~capture ~f =
      if capture
      then enter_new_thread_with_capture env dbg ~parent_capture:None ~f
      else f env { dbg; capture = None; checkpoint = Not_needed }
    ;;
...
module Logged = struct
  ...
  let log_default sexp = prerr_endline (Sexp.to_string sexp)
...
let quote_for_errors s =
  ...
  String.iter s ~f:(function
    ...
    | '_' | '-' | ':' | '.' | '/' | ',' | '+' | '=' | '%' | '@' -> ()
    | _ -> need_quoting := true);
  if !need_quoting then Filename.quote s else s
;;
...
module Background_command = struct
  ...
  let create pid = { mutex = Mutex.create (); pid; wait = lazy (waitpid pid) }
  let pid t = t.pid

  let wait t =
    Mutex.lock t.mutex;
    protectx t.mutex ~finally:Mutex.unlock ~f:(fun _ -> Lazy.force t.wait)
  ;;
end
...
let run_exit_code prog args =
  run_exit_status prog args
  >>| function
  | Exited n -> n
  | Signaled signal ->
    Printf.ksprintf
      failwith
      "Command got signal %s: %s"
      (Signal.name signal)
      (cmd_line prog args)
;;

let run prog args =
  run_exit_code prog args
  >>| fun code ->
  if code <> 0
  then
    Printf.ksprintf failwith "Command exited with code %d: %s" code (cmd_line prog args)
;;

let run_bool ?(true_v = [ 0 ]) ?(false_v = [ 1 ]) prog args =
  run_exit_code prog args
  >>| fun code ->
  if List.mem code ~set:true_v
  then true
  else if List.mem code ~set:false_v
  then false
  else
    Printf.ksprintf
      failwith
      "Command exited with unexpected code %d: %s"
      code
      (cmd_line prog args)
;;
...
let cwd_logical =
  let prim = Prim.make "cwd" [] (F sexp_of_string) Env.cwd_logical in
...
let read_all =
  let prim =
    Prim.make "read-all" [] (F sexp_of_string) (fun env ->
      Bigstring.read_all (Env.stdin env))
  in
  pack0 prim
;;
...
let fold_lines : type a. init:a -> f:(a -> string -> a t) -> a t =
  ...
  fun ~init ~f ->
    Fold
      ...
      ; init
      ; f
      ; fold = (fun env ~init ~f -> Bigstring.fold_lines (Env.stdin env) ~init ~f)
      }
;;

let fold_chunks =
  let prim =
    { Prim.name = "read-chunk"
  ...
  fun ~sep ~init ~f ->
    Fold
      ...
      ; init
      ; f
      ; fold = (fun env ~init ~f -> Bigstring.fold_chunks (Env.stdin env) ~sep ~init ~f)
      }
;;
...
let create_pipe =
  let prim =
    Prim.make
      "create-pipe"
      []
      (F (Sexp.pair Posixat.Fd.sexp_of_t Posixat.Fd.sexp_of_t))
...
let close_fd =
  let prim =
    Prim.make "close-fd" [ A Posixat.Fd.sexp_of_t ] Unit (fun _ fd -> Unix.close fd)
  in
...
let open_file =
  ...
  in
  fun ?(perm = 0) ~flags fn -> pack3 prim perm flags fn
;;

let redirect ios ?perm ~flags fn t =
  open_file ?perm ~flags:(List.map ~f:Posixat.Open_flag.of_unix_open_flag flags) fn
  >>= fun fd -> protect ~finally:(close_fd fd) (set_ios ios fd t)
;;

let std_to ios ?append fn t =
  redirect
    ios
    ~flags:[ O_WRONLY; O_CREAT; (if append = None then O_TRUNC else O_APPEND) ]
    ~perm:0o666
    fn
    t
;;
...
let replace_io =
  let prim =
    { Prim.name = "replace-io"
    ; args =
        [ O ("stdin", Std_io.sexp_of_t, Stdin)
...
let chmod =
  let prim =
    Prim.make
      "chmod"
      [ A sexp_of_string; L ("perm", Posixat.File_perm.sexp_of_t) ]
      Unit
...
let chown =
  let prim =
    Prim.make
      "chown"
      [ A sexp_of_string; L ("uid", sexp_of_int); L ("gid", sexp_of_int) ]
      Unit
...
let rm =
  let prim = Prim.make "rm" [ A sexp_of_string ] Unit Env.rm in
  fun fn -> pack1 prim fn
;;

let rmdir =
  let prim = Prim.make "rmdir" [ A sexp_of_string ] Unit Env.rmdir in
  fun fn -> pack1 prim fn
;;

let mkfifo =
  if Sys.win32 || Posixat.has_mkfifoat
  then (
    let prim =
      Prim.make
        "mkfifo"
        [ O ("perm", Posixat.File_perm.sexp_of_t, 0o666); A sexp_of_string ]
        Unit
...
let link =
  let prim = Prim.make "link" [ A sexp_of_string; A sexp_of_string ] Unit Env.link in
  fun oldpath newpath -> pack2 prim oldpath newpath
;;

let rename =
  let prim = Prim.make "rename" [ A sexp_of_string; A sexp_of_string ] Unit Env.rename in
  fun oldpath newpath -> pack2 prim oldpath newpath
;;

let symlink =
  let prim =
    Prim.make "symlink" [ A sexp_of_string; A sexp_of_string ] Unit Env.symlink
  in
  fun oldpath newpath -> pack2 prim oldpath newpath
;;

let stat =
  let prim = Prim.make "stat" [ A sexp_of_string ] (F Posixat.Stats.sexp_of_t) Env.stat in
  fun path -> pack1 prim path
;;

let lstat =
  let prim =
    Prim.make "lstat" [ A sexp_of_string ] (F Posixat.Stats.sexp_of_t) Env.lstat
  in
  fun path -> pack1 prim path
;;

let readlink =
  let prim = Prim.make "readlink" [ A sexp_of_string ] (F sexp_of_string) Env.readlink in
  fun path -> pack1 prim path
;;

let readdir =
  let prim =
    Prim.make "readdir" [ A sexp_of_string ] (F (sexp_of_list sexp_of_string)) Env.readdir
  in
  fun path -> pack1 prim path
;;
...
let file_exists =
  let prim =
    Prim.make "file-exists" [ A sexp_of_string ] (F sexp_of_bool) (fun env path ->
...
let temp_dir =
  let prim = Prim.make "temp-dir" [] (F sexp_of_string) get_temp_dir in
  pack0 prim
;;

let set_temp_dir =
  let prim =
    { Prim.name = "set-temp-dir"
    ; args = [ A sexp_of_string ]
    ; result = Env
...
module Temp = struct
  ...
  let create_dir = create "directory" (fun env path -> Env.mkdir env path ~perm:0o700)
end

let with_temp_file ~prefix ~suffix f =
  Temp.create_file ~prefix ~suffix >>= fun fn -> protect ~finally:(rm fn) (reify_exn f fn)
;;

let with_temp_dir ~prefix ~suffix f =
  Temp.create_dir ~prefix ~suffix
  >>= fun dn -> protect ~finally:(rm_rf dn) (reify_exn f dn)
;;
...
let sleep =
  let prim = Prim.make "sleep" [ A sexp_of_float ] Unit (fun _ d -> Unix.sleepf d) in
  fun d -> pack1 prim d
;;
