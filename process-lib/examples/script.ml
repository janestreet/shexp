...
let command (exe : string) : bool t = find_executable exe >>| fun exe -> exe <> None

(* Finds all files with the given extension in the git repo (or subdirs) *)
let find_files ~(ext : string) : unit t =
  fork (command "git") (command "grep")
  >>= function
  | true, true -> run "git" [ "ls-files" ] |- run "grep" [ Printf.sprintf "\\.%s$" ext ]
  | git, grep ->
    (if git
     then echo "Bummer, grep not found in PATH"
     else if grep
     then echo "Bummer, git not found in PATH"
...
let rec countdown (n : int) : unit t =
  if n > 0
  then echo (string_of_int n) >> sleep 1.0 >> countdown (n - 1)
  else echo "We'll meet again, Don't know where, don't know when..."
;;
